...
開発プロセスの経験の幅（ウォーターフォール・スクラム・カンバン）があり、それらへの理解も一定以上のものがあると自負しています。ウォーターフォールについては組織の標準プロセスおよびドキュメントフォーマットの策定を行っていましたし、スクラムにおいては組織導入のパイロットチームへの参画＆組織展開を推進しました。
